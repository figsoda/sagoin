...
use crate::{cli::Opts, state::State, Props, PropsExt};

impl<W: Write> State<W> {
    pub(crate) fn negotiate_otp(&mut self, props: &Props, opts: &Opts) -> Result<Props> {
        match props.get_prop("authentication.type")?.as_str() {
            "ldap" => {
                writeln!(self.out, "Authenticating with ldap")?;
                let user = self.resolve_username(opts)?;
                let pass = self.resolve_password(opts)?;
